...
export function PlatformWaysSlider({
  products,
}: {
  ...
  useLayoutEffect(() => {
      ...
      tween = gsap.to(track, {
        ...
        scrollTrigger: {
          ...
          start: "top top",
            ...
            `+=${Math.max(maxX() * 1.15, window.innerHeight * 1.35)}`,
          pin: true,
          scrub: 0.55,
          anticipatePin: 1,
    ...
    const onResize = () => {
      if (mq.matches) ScrollTrigger.refresh();
    };
  ...
  return (
    <div className="relative mt-4">
      {/* Mobile / tablet portrait: stacked cards, no pin */}
      <div className="space-y-4 md:hidden">
        {products.map((product, i) => (
          <WayCard key={product.href} product={product} index={i} />
        ))}
      </div>

      {/* md+: horizontal scroll-pin slider */}
