...
const ctxMap = new WeakMap<HTMLElement, TunnelCtx>();
...
function writeUniforms(u: UniformMap, props: Required<Omit<LightTunnelProps, "className" | "onReady">>) {
  ...
  u.uColorVariance.value = props.colorVariance ? 1.0 : 0.0;
...
export default function LightTunnel({
  ...
  useEffect(() => {
    ...
    writeUniforms(program.uniforms, propsRef.current);
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    writeUniforms(ctx.program.uniforms, propsRef.current);
  }, [
    cableColor,
    pulseColor,
    tunnelColor,
