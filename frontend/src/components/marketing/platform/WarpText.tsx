...
const wordGradient = (
  ...
  const top = y - fontSizePx * 0.46;
  const bottom = y + fontSizePx * 0.38;
  const gradient = ctx.createLinearGradient(x, top, x + width * 0.02, bottom);
...
const drawLine = (
  ...
  const marked = new Set(gradientWords.map((word) => word.toLowerCase()));
  const tokens = line.split(/(\s+)/);
  let cursor = x - measureLine(ctx, line, letterSpacing) / 2;

  tokens.forEach((token) => {
    if (!token) return;
    const width = measureLine(ctx, token, letterSpacing);
    const isWord = !/^\s+$/.test(token) && marked.has(token.toLowerCase());
    ctx.fillStyle = isWord
      ? wordGradient(ctx, cursor, y, width, fontSizePx, gradientColors)
      : color;
    drawChars(ctx, token, cursor, y, letterSpacing);
    cursor += width;
  });
};
...
export default function WarpText({
  ...
  useEffect(() => {
    ...
    const renderOnce = () => {
      if (disposed || contextLost || !renderer || !mesh) return;
    ...
    const rasterize = async () => {
      ...
      if (disposed || contextLost || version !== rasterVersion || !texture) return;
    ...
    const resize = () => {
      if (disposed || contextLost || !renderer || !program || !gl) return;
      const rect = container.getBoundingClientRect();
      if (rect.width <= 0 || rect.height <= 0) return;
